...
Before the first content command in an agent session, require agy-search 0.2.8
...
- Ordinary work omits both `agy-search models` and `--model`. CLI 0.2.8
...
When no publishable result survives, Standard Search performs at most two
quality-model recoveries under the original command deadline. Do not add an
outer retry for the same broad command.
