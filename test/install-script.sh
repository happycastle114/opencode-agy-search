...
archive="$e2e_root/assets/opencode-agy-search-0.3.9.tgz"
mv "$packed" "$archive"
...
perl -0pi -e 's/"version": "0\.3\.9"/"version": "0.2.9"/' \
...
sed 's/PACKAGE_VERSION="0.3.9"/PACKAGE_VERSION="0.2.9"/' \
...
preexisting_release="$e2e_root/preexisting-data/opencode-agy-search/releases/0.3.9-${archive_digest:0:12}"
...
assert_preexisting_mode_rejected() {
  ...
  local mode_release="$mode_data/opencode-agy-search/releases/0.3.9-${archive_digest:0:12}"
...
printf '{"name": "@happycastle114/opencode-agy-search", "version": "0.3.9"}\n' \
